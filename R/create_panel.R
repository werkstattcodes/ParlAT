#' Create Panel
#'
#' `create_panel` creates a
#' panel of MPs with their mandates for a given legislative period and institution.
#'
...
create_panel <- function(legis_period = NULL, institution = NULL) {
  ...
  #unique pad_interns
  vec_pad_intern <- df_mps_all %>% dplyr::distinct(.data$pad_intern) %>% dplyr::pull()
  # return(vec_pad_intern)

  #get all their mandates in the respective institution
  df_mandates <- vec_pad_intern %>%
    purrr::map(
      \(x) get_mandates(pad_intern = x, institution = institution),
      .progress = "Get MPs' mandates"
    ) %>%
    purrr::list_rbind()
  ...
  df_mandates <- df_mandates %>%
    dplyr::mutate(
      mandatBis = dplyr::case_when(
        is.na(.data$mandatBis) & .data$aktiv == TRUE ~ lubridate::today(),
        .default = .data$mandatBis
      )
    )
  ...
  df_names <- purrr::map(
    vec_pad_intern,
    \(x) get_names(pad_intern = x),
    .progress = "Get MPs name variants"
  ) %>%
    purrr::list_rbind()
  ...
  df_names <- df_names %>%
    dplyr::mutate(
      date_start = dplyr::case_when(
        is.na(.data$date_start) ~ lubridate::today() - (100 * 365),
    ...
    dplyr::mutate(
      ...
      df_mandates_long_pad_fill_names <- df_mandates_long_pad_fill %>%
        dplyr::left_join(
          ...
          by = dplyr::join_by(
            "pad_intern",
            between("mandate_date", "date_start", "date_end")
          )
        ) %>%
      ...
      df_legis_period <- get_legis_periods(legis_period = {{ legis_period }})
  
      df_legis_period <- df_legis_period %>%
        dplyr::mutate(
          date_end = dplyr::case_when(
            .data$legis_period_current == TRUE ~ lubridate::today(),
            .default = .data$date_end
          )
        )
      ...
      if (!is.null(legis_period)) {
        df_mandates_long_pad_fill_names <- df_mandates_long_pad_fill_names %>%
          dplyr::semi_join(
            df_legis_period,
            by = dplyr::join_by(between("mandate_date", "date_start", "date_end"))
          )
