...
- [`get_party_colors()`](https://werkstattcodes.github.io/ParlAT/reference/get_party_colors.md)
...
- `aux_check_pad_intern_exists()` now uses
  [`httr2::req_retry()`](https://httr2.r-lib.org/reference/req_retry.html)
  for its preliminary person-page check, allowing at most three total
  attempts for retryable responses before an identifier is treated as
  unavailable.
...
- Stale hardcoded browser cookies/session IDs, Chrome/Edge client hints,
  Fetch Metadata headers, and browser user-agent strings were removed
  from requests.
